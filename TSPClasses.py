...
class Colony:
    ...
    def release_ants(self):
        ...
        for i in range(self.num_ants):
            ants.append(Ant(self.start_city, self.cities_to_visit, self.decent_route_average,
                            self.optimal_route_pherimone_count, self.pherimone_map))
        while ants:
            for ant in ants:
                if (ant.action == None):
                    ants.remove(ant)
        ...
        if child_index_left < len(self.queue):  # If left child exists
            if self.queue[child_index_left].get_priority() < check_val:  # If left child less than parent
                if child_index_right < len(self.queue):  # If right child exists
                    if self.queue[child_index_right].get_priority() < self.queue[
                        child_index_left].get_priority():  # If right child less than left
